...
@section('title', 'Audit Trail di Sicurezza')

@section('content')
<div class="card">
    <div class="card-header">
        <div>
            <h1 class="card-title">📜 Audit Trail di Sicurezza</h1>
            <p style="color: var(--text-muted); font-size: 0.85rem; margin-top: 0.25rem;">
                Registro immutabile delle assegnazioni e revoche di ruoli e permessi per conformità e monitoraggio degli accessi.
            </p>
        </div>
    </div>
    ...
    @if(! $tableExists)
        <div style="background: rgba(245, 158, 11, 0.1); border: 1px solid var(--warning); padding: 1.25rem; border-radius: 0.375rem; margin-bottom: 1.5rem;">
            <h3 style="color: var(--warning); font-size: 1rem; margin-bottom: 0.5rem;">Tabella Audit Log non ancora migrata</h3>
            <p style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 0.75rem;">
                Per attivare il tracciamento automatico di sicurezza, pubblica ed esegui la migrazione della tabella di audit:
            </p>
    ...
    @else
        <form method="GET" action="{{ route('permission-toolkit.audit') }}" style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 200px;">
                <input type="text" name="search" class="input-control" value="{{ request('search') }}" placeholder="Cerca per bersaglio o IP...">
            </div>
            ...
            <div style="width: 160px;">
                <select name="type" class="input-control">
                    ...
                    <option value="role" {{ request('type') == 'role' ? 'selected' : '' }}>Ruolo</option>
                    <option value="permission" {{ request('type') == 'permission' ? 'selected' : '' }}>Permesso</option>
                    <option value="role_permission" {{ request('type') == 'role_permission' ? 'selected' : '' }}>Ruolo ➔ Permesso</option>
                </select>
            </div>
            <button type="submit" class="btn">Filtra</button>
            @if(request()->hasAny(['search', 'action', 'type']))
                <a href="{{ route('permission-toolkit.audit') }}" class="btn" style="background: var(--border);">Reset</a>
            @endif
        </form>
                    ...
                    @forelse($logs as $log)
                        <tr>
                            ...
                            <td>
                                ...
                                @else
                                    <span style="color: var(--text-muted);">Sistema / CLI</span>
                                @endif
                            </td>
                            <td>
